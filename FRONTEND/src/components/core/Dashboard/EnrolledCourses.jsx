...
export default function EnrolledCourses() {
  ...
  return (
    <div>
      ...
      {!enrolledCourses ? (
        <div className="flex items-center justify-center h-[calc(100vh-3rem)] text-richblack-5">Loading...</div>
            ...
            return (
              <div className="flex items-center" key={index}>
                <div className="flex text-richblack-5 w-[45%] items-center gap-4 py-4 px-2">
                  <img src={course.thumbnail} className="h-14 w-14 object-cover" />
                  <div>
                    <p className="font-semibold">{course.name}</p>
                    <p className="text-xs text-richblack-300 hidden md:block">{course.description}</p>
                  </div>
                </div>
                <div className="text-richblack-5 w-1/4 md:px-4 self-center">{course.totalDuration || "1hr20min"}</div>

                <div className="w-1/5 ">
                  <p className="mb-1 text-richblack-5">Progress: {course.progressPercentage || 0}%</p>
                  <ProgressBar
